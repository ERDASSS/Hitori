...
class Interactive(State):
    ...
    def handle_triangle(screen, height):
        try:
            grid = Generator.generate_triangle_grid(height)
        except Exception as e:
            screen.addstr(1, 0, f"Ошибка генерации сетки: {str(e)}")
            screen.refresh()
            screen.getch()
        ...
        while True:
            ...
            key = screen.getch()
            if key == curses.KEY_UP and cursor_row > 0 and cursor_row != cursor_col:
                cursor_row -= 1
            elif key == curses.KEY_DOWN and cursor_row < len(grid) - 1:
                cursor_row += 1
            elif key == curses.KEY_LEFT and cursor_col > 0:
                cursor_col -= 1
            elif key == curses.KEY_RIGHT and cursor_col < len(grid[cursor_row]) - 1:
                cursor_col += 1
            elif key == ord(" "):
                Display.toggle_cell(grid, original_grid, cursor_row, cursor_col)
            elif key in (ord("q"), ord("Q"), ord("й"), ord("Й")):

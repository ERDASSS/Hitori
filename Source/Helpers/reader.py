...
class Reader:
    ...
    @staticmethod
    def get_row_input_triangle(screen, row_number: int, max_len: int) -> str:
        """
		Получает ввод строки от пользователя.
		"""
        screen.addstr(row_number + 1, 0, f"Строка {row_number + 1}: ")
        ...
        row_input = ""
        cursor_pos = len(f"Строка {row_number + 1}: ") + max_len - row_number + 1
        while True:
            key = screen.getch()
            if key in (10, 13):  # Enter
                break
            elif key in (8, 127, curses.KEY_BACKSPACE):  # Backspace
                if cursor_pos > len(f"Строка {row_number + 1}: ") + max_len - row_number + 1:
                    row_input = row_input[:-1]
                    cursor_pos -= 1
                    screen.addstr(row_number + 1, cursor_pos, " ")
                    screen.refresh()
                ...
                row_input += chr(key)
                cursor_pos += 1
            screen.addstr(row_number + 1, len(f"Строка {row_number + 1}: ") + max_len - row_number + 1, row_input)
            screen.refresh()
